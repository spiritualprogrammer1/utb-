...
class StockController extends Controller
{
    ...
    public function output()
    {
        ...
        $movements = Movement_stock::where('type', '0')->whereYear('created_at', date('Y'))
            ->whereMonth('created_at', date('m'))->orderBy('created_at', 'desc')->get();
        return view('stock.output', ['movements' => $movements, 'stocks' => $stocks]);
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->has("delivery")) {
            ...
            $messages = array(
                'command.required' => "Le Numéro de Commande est obligatoire ",
                'reference.required' => "La Reférence du stock est obligatoire ",
                'quantity.required' => "La Quantité est requise",
            );
            $rules = array(
                'command' => 'bail|required|min:1|max:255',
            ...
            if ($validator->fails()) {
                ...
                return response()->json(['id' => $movement->id, 'reference' => $movement->reference,
                    'command' => $movement->command, 'quantity' => $movement->item_stock->sum('quantity'),
    ...
    public function movement(Request $request, $id, $type)
    {
        if ($request->ajax()) {
            if ($type == "output") {
                ...
                return view('stock.partials.output', ['items' => $items, 'movement' => $movement]);
            } elseif ($type == "back"){
                $movement = Movement_stock::findOrFail($id);
                $items = Item_stock::where('movement_stock_id', $id)->get();
    ...
    public function search(Request $request)
    {
        if ($request->ajax()) {
            if ($request->has('output')) {
                ...
                $movements = Movement_stock::whereBetween('created_at', [$begin, $end])->where('type', '0')
                    ->orderBy('created_at', 'desc')->get();
                return view('stock.includes.output', ['movements' => $movements]);
