                ...
                <header class="header bg-white b-b clearfix">
                    <div class="row m-t-sm">
                        <div class="col-sm-6 m-b-xs">
                            ...
                            <?php $count= count($insurance_expiration); ?>
                            <h5><i class="fa fa-bus"></i> LISTE DES CARS DONT L'ASSURANCE EXPIRE BIENTOT <i class="i i-asc"></i> : <b style="color: #002a80">{{$count}} véhicule(s)</b>  </h5>
                        ...
                        <div class="table-responsive" id="view">
                            <table class="table table-striped m-b-none capitalize"
                                ...
                                <tr>
                                    ...
                                    <th>Marque</th>
                                    <th>Modele</th>
                                    <th><i class="i i-calendar text-warning"></i> Assurance</th>
                                </tr>
                                </thead>
                                <tbody id="vehicleRow">
                                @for($i=0;$i<$count;$i++)
                                    <tr id="bus{{$insurance_expiration[$i]['id']}}" class="animated fadeIn">
                                        <td>{{$insurance_expiration[$i]['designation']}}</td>
                                        <td class="text-info-dk uppercase">{{$insurance_expiration[$i]['matriculation']}}</td>
                                        <td class="text-info-dk uppercase">{{$insurance_expiration[$i]['chassis']}}</td>
                                        <td>{{$insurance_expiration[$i]->model->brand->name}}</td>
                                        <td>{{$insurance_expiration[$i]->model->name}}</td>
                                        <?php $assurance=App\Assurance::where('bus_id',$insurance_expiration[$i]->id)->orderBy('id','DESC')->first();
                                        ?>
                                        <td class="text-danger">{{\Jenssegers\Date\Date::parse($assurance['date'])->format('j M Y')}}</td>
