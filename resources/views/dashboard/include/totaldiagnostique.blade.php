                ...
                <div class="row">
                    <table class="table table-responsive table-stripped" id="vehicleTable">
                        ...
                        @foreach($data as $key=> $data)
                            <tr id="contractor">
                                <td>{{$key+1}}</td>
                                <td class="text-danger">{{strtoupper($data->statee->bus->societe)}}</td>
                                <td>{{strtoupper($data->statee->bus->matriculation)}}</td>
                                <td style="text-transform: capitalize">{{$data->statee->bus->model->name." ".$data->statee->bus->model->brand->name}}</td>
                                <td>{{$data->statee->bus->chassis}}</td>
