                ...
                <div class="row">
                    <table class="table table-responsive table-stripped" id="vehicleTable">
                        ...
                        @foreach($data as $key=> $data)
                            <tr id="contractor">
                                <td>{{$key+1}}</td>
                                <td>{{strtoupper($data->proforma_invoice->reference)}}</td>
                                <td style="text-transform: capitalize">{{$data->process->state->vehicle->client->name}}</td>
                                <td>{{$data->process->state->vehicle->matriculation}}</td>
