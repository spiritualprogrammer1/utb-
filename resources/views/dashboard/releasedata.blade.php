                ...
                <div class="row">
                    <table class="table table-responsive table-stripped" id="vehicleTable">
                        ...
                        @foreach($data as $key=> $data)
                            <tr id="contractor">
                                <td>{{$key}}</td>
                                <td class="alert alert-warning">{{strtoupper($data->diagnostic->statee->reference)}}</td>
                                <td>{{strtoupper($data->diagnostic->statee->bus->matriculation)}}</td>
                                <td style="text-transform: capitalize">{{$data->diagnostic->statee->bus->model->name." ".$data->diagnostic->statee->bus->model->brand->name}}</td>
                                <td>{{$data->diagnostic->statee->bus->chassis}}</td>
