        ...
        <table class="table table-responsive table-bordered" style="border: 2px solid; border-collapse:collapse;">
            <tr style="border-bottom: 2pt solid;">
                <td class="text-center uppercase"> N° OT <br>
                    {{$revision->diagnostic->statee->reference}}
                </td>
                <td class="text-center" colspan="2" style="border-left: 2pt solid;border-right: 2pt solid; background-color: rgba(0, 8, 0, 0.2); "><b>FICHE DE REVISION<br>CAR<b></b></td>
                <td class="text-center uppercase">N° VEHICULE<br>
                    {{$revision->diagnostic->statee->bus->matriculation}}
                </td>
            </tr>
            <tr style="border-bottom: 2pt solid">
                <td rowspan="3" class="border_bottom_2" >
                    <p>
                        ...
                        <br>
                        {{number_format($revision->diagnostic->statee->kilometer)}}</p>
                    <p>
                ...
                <td colspan="1" class="text-center" style="border-left: 2px solid; border-right: 2px solid">
                    <small>DATE DERNIERE REVISION</small>
                </td>
                ...
                <td rowspan="3" >
                    <p>Date<br>
                        <small> {{\Jenssegers\Date\Date::parse($revision->created_at)->format('j M Y')}}</small>
                    </p>
                    <p>Garage<br>UTB</p>
                </td>
            </tr>
            <tr style="border-top: 1px solid; border-bottom: 1px solid">
                @foreach($denierrevision as  $denierrevisio)
                    <td>{{$denierrevisio->date}}</td>
                @endforeach
            </tr>
            <tr style="border-top: 1px solid; border-bottom: 1px solid">
                @foreach($denierrevision as  $denierrevisio)
                    <td>{{$denierrevisio->kilometer}}</td>
                @endforeach
            </tr>
            ...
            @foreach ($revision->diagnostic->service_description as $description)
