@section('title') Retour de pieces @endsection
@section('styles')
    <style>
        @media print {
            .row {
...
@section('content')
    <section class="hbox stretch">
        <aside class="aside-lg bg-light dker b-r" id="subNav">
            <section>
                <form method="post" id="backForm"  role="form" class="panel b-a bg-light lter">
                    {{csrf_field()}}
                    <h5 class="uppercase m-l-sm">Retour de pieces
                        <button type="submit" id="submit_back"
                                class="btn btn-success btn-sm btn-rounded pull-right m-r-sm m-t-n-xs"
                                title="Valider le retour de pieces">
                            <i class="fa fa-check-circle"></i> VALIDER
                        </button>
                        <a id="add_back" class="btn btn-icon btn-info btn-sm btn-rounded pull-right m-r-md m-t-n-xs"
                    ...
                    <div class="panel-body panel-default">
                        <div class="row">
                            <table class="table table-striped" id="back_table">
                                <thead>
                                <tr>
                                ...
                                <tr>
                                    <td>
                                        <select class="input-sm form-control" id="reference_back"
                                        ...
                                        <p class="form-control-static text-center">
                                            <span id="info_back_family0">...</span>
                                        </p>
                                    </td>
                                    <td>
                                        <input class="form-control input-sm" name="quantity[]" id="quantity_back"
                                               type="number" placeholder="0" data-id="0" onkeyup="calcul($(this))"
                                               required>
                                        <input id="quantity_count0" type="hidden" value="0">
                                        <p class="form-control-static text-center">
                                            <span id="info_back_qty0" class="qty">...</span>
                                        </p>
                                    </td>
                                </tr>
                                </tbody>
                ...
                <header class="header bg-white b-b clearfix">
                    <div class="row m-t-sm">
                        <div class="col-sm-4 m-b-xs">
                            ...
                            <h4><i class="fa fa-sign-in"></i> RETOUR DE PIECES</h4>
                        </div>
                ...
                <section class="scrollable wrapper">
                    <section class="panel panel-default">
                        <div class="table-responsive" id="result">
                            <table class="table datatable table-striped b-t b-light" id="backTable">
                                <thead>
                                <tr>
                                    <th width="10"></th>
                                    <th>Reference</th>
                                ...
                                <tbody class="capitalize" id="backRow">
                                @foreach($movements as $key => $movement)
                                    <tr id="back{{$movement->id}}">
                                        <td>
            ...
            <script>
                ...
                $(function () {
                    ...
                    $add_back.on('click', function () {
                        var $table = $('#back_table tbody');
                        $table.append($('#back_table tbody tr:last').clone());
                        var rows = $('#back_table tr');
                        ...
                        text_select.eq(0).attr('id', 'reference_back' + count);
                        text_select.eq(0).attr('data-id', count);
                        text_span.eq(0).attr('id', 'info_back_family' + count);
                        text_span.eq(1).attr('id', 'info_back_qty' + count);
                        ...
                        $('#info_back_qty' + count).html('...');
                        $('#info_back_family' + count).html('...');
                    });
                    $form_back.on('submit', function (e) {
                        e.preventDefault();
                        var formData = $(this).serialize();
                        var type = 'post';
                        var url = 'stock';
                        status = "success";
                        var msg = "Le Retour de pieces a bien été valider";
                        $submit_back.button({loadingText: '<i class="fa fa-spinner fa-spin"></i> en cours...'});
                        $submit_back.button('loading');
                        $.ajax({
                            url: url,
                            type: type,
                            data: formData,
                            success: function (data) {
                                $form_back.trigger('reset');
                                toastr[status](msg, "<span class='uppercase'>Retour de pieces</span>!");
                                toastr.options.preventDuplicates = true;
                                var row = '<tr id="back' + data.id + '" class="alert alert-info text-danger-dk">' +
                ...
                function infoReference(obj) {
                    var ids = $(obj).val(),
                        id = $(obj).attr('data-id'),
                        info_back_family = $('#info_back_family' + id),
                        qty_count = $('#quantity_count' + id),
                        info_back_qty = $('#info_back_qty' + id),
                        type = "back";
                    $.get('references/' + ids + '/' + type, function (data) {
                        info_back_family.html(data.family + " - " + data.subfamily);
                        info_back_qty.html(data.qty);
                ...
                function calcul(obj) {
                    var id = $(obj).attr('data-id'),
                        ...
                        quantity = $('#info_back_qty' + id);
                    result = parseInt(qty) + parseInt(val);
                    quantity.html(result);
                    if (result < 0) {
                        $submit_back.addClass('disabled');
                        quantity.addClass('text-danger')
                    } else {
                        $submit_back.removeClass('disabled');
